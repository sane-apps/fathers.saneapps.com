...
1b. **Author dates required.** Every public author name used as a heading or list label shows lifespan/floruit next to the name — Authors index, author hubs, and Works catalogue chronology/author-sort group headings (`data/author-dates.json` + `authors.json`). Format: `1572–1641 AD` when known; else `fl. c. … AD` / `c. …–c. … AD`. Use **BC** and **AD**, never CE or BCE. Example: Nemesius of Emesa, fl. c. 390 AD. Authors lists are chronological (earliest first), not A–Z. Do not hide dates on Works and show them only on Authors.
